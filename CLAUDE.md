...
`docs/llms.md` 是给 AI 编程助手(Claude Code / Cursor / Copilot 等)读的单文件组件参考。它**不手写**，由 `scripts/gen-llms-doc.mjs` 从 `playground/sections/*.tsx` 的 `demos` 和 `api` 字段自动生成。

- 改过 playground demo 后，跑一次 `npm run gen:docs` 刷新 `docs/llms.md`。
- 发布流程 (`prepublishOnly`) 已经内置这一步，手动 `npm publish` 时会自动重新生成。
- 不要手工编辑 `docs/llms.md`，改动会在下次生成时被覆盖。真正的"源"是 playground 的 `defineSection({ ... })` 参数 + 组件内 `<DocPage demos api />` 传入的对象字面量。
- 根目录的 `llms.txt` 是给 LLM crawler 看的索引(遵循 <https://llmstxt.org/> 规范)，指向 `docs/llms.md` 的 raw GitHub URL。
